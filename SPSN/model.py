...
class MyModel(nn.Module):
    ...
    def forward(self, image, depth, rgb_ss_map, depth_ss_map):
        ...
        feat_56 = self.rgb_merge_co_56(torch.cat([tortal_sampled_feature5, resize(tortal_sampled_feature6, [16, 16])], dim=1))                # feat_56 : [N, 128, 14, 14]
        feat_45 = self.rgb_merge_co_45(torch.cat([tortal_sampled_feature4, resize(feat_56, [32, 32])], dim=1))                         # feat_45 : [N, 128, 28, 28]
...
if __name__ == '__main__':
    model = MyModel()
    rgb = torch.rand(5, 3, 352, 352)
    depth = torch.rand(5, 3, 352, 352)
    rgb_ssmap = torch.rand(5, 100, 352, 352)
    depth_ssmap = torch.rand(5, 100, 352, 352)
    ...
    flops, params = profile(model, inputs=(rgb, depth, rgb_ssmap, depth_ssmap))
